...
export function findPhoneCountry(code: string | null | undefined): PhoneCountry {
  const match = PHONE_COUNTRIES.find(
    (country) => country.code === code?.toUpperCase(),
  );
  return match ?? PHONE_COUNTRIES.find((c) => c.code === "US")!;
}

export function guessDefaultPhoneCountry(): string {
  if (typeof navigator === "undefined") return "US";
  const region = navigator.language?.split("-")[1]?.toUpperCase();
  if (region && PHONE_COUNTRIES.some((c) => c.code === region)) {
    return region;
  }
  return "US";
}
